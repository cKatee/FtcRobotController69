...
public class RedLeftRefactored extends auto {
    ...
    private PoseStablizationController poseStablizationController = new PoseStablizationController(robot);
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            switch (AUTO_STATE) {
                case START:
                    poseStablizationController.updateMovement(start_position);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(4)) {
                        AUTO_STATE = auto_states.DRIVE_TO_AVOID_STARTING_STACK;
                    }
                    break;
                case DRIVE_TO_AVOID_STARTING_STACK:
                    poseStablizationController.updateMovement(high_goal_so_we_miss_starting_stack);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(2.5)) {
                ...
                case DRIVE_TO_POWER_SHOT:
                    poseStablizationController.updateMovement(high_goal_general_position);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(2)) {
                ...
                case SHOOT_FIRST_POWER_SHOT:
                    robot.shooter.setVelocity(selectedShootingTPM);
                    poseStablizationController.updateMovement(high_goal_1);
                    if (((robot.shooter.getVelocity() / 28) * 60) > selectedShootingRPM - 80  && poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(2.1)) {
                ...
                case SHOOT_SECOND_POWER_SHOT:
                    robot.shooter.setVelocity(selectedShootingTPM);
                    poseStablizationController.updateMovement(high_goal_2);
                ...
                case SHOOT_THIRD_POWER_SHOT:
                    poseStablizationController.updateMovement(high_goal_3);
                ...
                case DRIVE_TO_WOBBLE_ZONE:
                    poseStablizationController.updateMovement(new position(wobble_goal_spot.getX() + 5,wobble_goal_spot.getY() + 2,wobble_goal_spot.getAngleRadians()));
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(3)) {
                ...
                case GO_TO_SECOND_WOBBLE_GOAL_AVOID_STACK:
                    poseStablizationController.updateMovement(to_second_wobble_avoid_stack);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(4)) {
                        AUTO_STATE = auto_states.GO_TO_SECOND_WOBBLE_GOAL;
                    }
                    break;
                case GO_TO_SECOND_WOBBLE_GOAL:
                    to_second_wobble_avoid_stack.distanceToPose(second_wobble_goal);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(2)) {
                        robot.drive.STOP();
                ...
                case AVOID_RING_STACK_TO_WOBBLE_SPOT_AGAIN:
                    poseStablizationController.updateMovement(new position(to_second_wobble_avoid_stack.getX() , to_second_wobble_avoid_stack.getY() - 10, to_second_wobble_avoid_stack.getAngleRadians()));
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(3)) {
                        AUTO_STATE = auto_states.to_second_wobble_spot;
                    }
                    break;
                case to_second_wobble_spot:
                    poseStablizationController.updateMovement(on_way_to_second_wobble_spot);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(3)) {
                        AUTO_STATE = auto_states.DRIVE_TO_WOBBLE_ZONE_AGAIN;
                    }
                    break;
                case DRIVE_TO_WOBBLE_ZONE_AGAIN:
                    poseStablizationController.updateMovement(new position(wobble_goal_spot.getX() - 20,wobble_goal_spot.getY(),wobble_goal_spot.getAngleRadians()));
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(3)) {
                ...
                case AVOID_PLACED_GOALS:
                    ...
                    poseStablizationController.updateMovement(to_line_avoid_wobble_goals);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(4)) {
                ...
                case GO_INFRONT_OF_RING:
                    poseStablizationController.updateMovement(infront_of_ring_stack);
                    if (poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(3)) {
                        AUTO_STATE = auto_states.PICK_UP_SECOND_RING;
                    }
                    break;
                case PICK_UP_SECOND_RING:
                    poseStablizationController.updateMovement(ring_stack);
                ...
                case SHOOT_RING_AGAIN:
                    ...
                    poseStablizationController.updateMovement(shoot_starting_stack_position);
                    ...
                    if (((robot.shooter.getVelocity() / 28) * 60) > robot.secondSpeed - 100 && (System.currentTimeMillis() - time_of_extra_shot) > time_between_shots && poseStablizationController.isRobotWithinAllowedToleranceToSetpoint(2.5)) {
                ...
                case PARK_ON_LINE:
                    poseStablizationController.updateMovement(line);
                    robot.shooter.setVelocity(0);
